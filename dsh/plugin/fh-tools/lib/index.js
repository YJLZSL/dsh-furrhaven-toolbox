...
function exec(cmd, args, cwd, timeoutMs) {
    return new Promise((resolve) => {
        execFile(cmd, args, { cwd: cwd || process.cwd(), timeout: timeoutMs, windowsHide: true, maxBuffer: MAX_BUFFER }, (error, stdout, stderr) => {
            if (error) {
...
export function apply(ctx) {
    ...
    reg(defineTool({
        ...
        async execute(args) {
            if (!args.say)
                return '[fh_play] 交互式扮演请在终端运行 `fh play <slug>`；本工具用 --say 做单轮测试';
