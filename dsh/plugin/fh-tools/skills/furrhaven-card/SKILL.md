...
## 7. 官方组件思路（Charlin：changeMsg / getMsgContent 嵌套）

FD 平台组件桥接函数中，`changeMsg` / `getMsgContent` 是组件间协作的关键：

- **组件 A 调组件 B**：A 的 script 用 `getMsgContent()` 读取当前消息内容，处理后通过 `changeMsg` 写回，实现嵌套/联动面板（如 HUD 状态改变后同步刷新 vnMap/Choices）。
- **切换开场白/背景**：结合 `changeMsg` 与组件，可实现多开场切换：不同组件/按钮调用 `changeMsg` 替换消息内容，达到“可选择不同背景开场”的效果。
- 落地纪律：`changeMsg`/`getMsgContent` 在 iframe 模式是桥接 Promise，必须 `await`；参数名仍走 `.sh-data` 占位符协议；组件间不直接引用对方 DOM，只通过消息内容/本地存储通信。

## 8. 防 AI 味提示包（官方同类提示）

写卡/改稿时把下列提示并入系统指令或审阅要点（genre-furry 包默认启用的子项可用 `fh.config.yaml` 关闭）：

- *兽人设定*：题材规则只进入 `genre-furry` 包；非兽人卡不预填。
- 无折行号：正文不要出现折行编号/分点编号，回归小说表现。
- 减少心理：叙述层少上帝视角心理旁白，多用动作/微表情/对话暗示。
- 7k 字待：长文卡按目标字数配比写作，避免注水。
- 禁止发情：不发情/不擦边直写；亲密关系含蓄处理。
- 不结束：对话/选项保持“可继续”，不主动完结故事线。
- 不要“不是 A 是 B”句式：叙述层禁用该句式（lint 已查 `PROSE-NOTAB`）。
- 破折号限额、死物比喻、跨卡专属词雷区：继续按 `fh check` 规则执行。
